...
class P2c2pRequest extends AbstractHelper {
	...
	public function p2c2p_construct_request($parameter, $isLoggedIn) {
		...
		$secretKey = array_key_exists("secretKey", $this->objConfigSettings) ? $this->objConfigSettings['secretKey'] : '';
		$hash_value = $this->objP2c2pHashHelper->createRequestHashValue($this->arrayP2c2pFormFields, $secretKey);
	...
	private function generateP2c2pCommonFormFields($parameter) {
		...
		$merchant_id = array_key_exists("merchantId", $this->objConfigSettings) ? $this->objConfigSettings['merchantId'] : '';
		$currency = $this->getMerchantSelectedCurrencyCode();
		$default_lang = array_key_exists("toc2p_lang", $this->objConfigSettings) ? $this->objConfigSettings['toc2p_lang'] : 'en';
		...
		$this->arrayP2c2pFormFields["result_url_1"] = $this->getMerchantFrontReturnUrl();
		$this->arrayP2c2pFormFields["result_url_2"] = $this->getMerchantBackendReturnUrl();
		$this->arrayP2c2pFormFields["payment_option"] = "1"; // Pass by default Payment option as A
	...
	function setPaymentExpiryTime() {
		...
		$payment_expiry = array_key_exists("paymentExpiry", $this->objConfigSettings) ? $this->objConfigSettings['paymentExpiry'] : '8';
		...
		date_default_timezone_set($this->_configLocaleTimezone);
		$date = date("Y-m-d H:i:s");
		$strTimezone = date_default_timezone_get();
		$date = new \DateTime($date, new \DateTimeZone($this->_configLocaleTimezone));
	...
	function getMerchantFrontReturnUrl() {
	...
	//Get the merchant website return URL.
	function getMerchantBackendReturnUrl() {
		$baseUrl = $this->objStoreManagerInterface->getStore()->getBaseUrl();
		return $baseUrl . 'P2c2p/payment/notify2C2P';
	}
}