...
class MassRunFullReindex extends Indexer
{
    /**
     * {@inheritdoc}
     */
        ...
        if (!is_array($indexerIds)) {
            ...
            try {
                ...
                $this->messageManager->addSuccessMessage(
                    __('Full reindex for %1 indexer(s) are completed.', count($indexerIds))
                );
            } catch (\Exception $e) {
                $this->messageManager->addExceptionMessage(
                    $e,
                    __("We couldn't complete full reindex because of an error.")
                );
            }
        }
