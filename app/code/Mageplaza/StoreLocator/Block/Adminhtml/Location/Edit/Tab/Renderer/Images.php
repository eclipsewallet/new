...
class Images extends Widget
{
    ...
    protected function _prepareLayout()
    {
        ...
        $this->getUploader()->getConfig()->setUrl(
            $this->_urlBuilder->addSessionParam()->getUrl('mpstorelocator/location/upload')
        )->setFileField(
            'image'
        )->setFilters([
                          'images' => [
    ...
    public function getImagesJson()
    {
        ...
        if (is_array($value) && count($value)) {
            ...
            foreach ($images as $key => &$image) {
                $image['url'] = $this->_imageHelper->getBaseMediaUrl() . '/' . $this->_imageHelper->getMediaPath($image['file']);
                try {
